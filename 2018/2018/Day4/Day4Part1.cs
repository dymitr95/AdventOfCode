...
public class Day4Part1 : Part<int>
{
    public override int Run(string input)
    {
        ...
        var maxSleepingTime = 0;
        var maxSleepingGuardId = 0;
        
        foreach (var guard in guards)
        {
            var sleepingTime = guard.SleepingTime();
            if (sleepingTime <= maxSleepingTime) continue;
            maxSleepingTime = sleepingTime;
            maxSleepingGuardId = guard.Id;
        }

        var mostSleepingMinute = guards.First(g => g.Id == maxSleepingGuardId).MostSleepingMinute();
